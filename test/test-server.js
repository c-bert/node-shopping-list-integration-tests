...
describe("Shopping List", function() {
  ...
  //  3. add the `id` to `updateRecipesData`
...
describe("Recipes List", function() {
  ...
  it("should list recipe items on GET", function() {
    ...
    return chai
      ...
      .then(function(res) {
        ...
        expect(res.body.length).to.be.at.least(1);
        // each item should be an object with key/value pairs for "name", "ingredients".
  ...
  it("should update recipes on PUT", function() {
    // we initialize our updateRecipesData here and then after the initial
    // request to the app, we update it with an `id` property so
    // we can make a second, PUT call to the app.
    ...
    return (
      chai
        ...
        .get("/recipes")
        .then(function(res) {
          updateRecipesData.id = res.body[0].id;
          return chai
            .request(app)
            .put(`/recipes/${updateRecipesData.id}`)
            .send(updateRecipesData);
        })
